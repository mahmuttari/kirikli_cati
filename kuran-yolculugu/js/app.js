...
function retroYilan(durak, index) {
  const alan = retroKabuk("🐍 Yılan", "Yemi (🍎) topla, kuyruğa ve duvara çarpma!");
  ...
  let yem = yemKoy(), skor = 0, bitti = false;
  function yemKoy() {
    let p;
  ...
  function adim() {
    ...
    yilan.unshift(bas);
    if (bas.x === yem.x && bas.y === yem.y) { skor++; skorEl.textContent = "Skor: " + skor; tonCal([660]); yem = yemKoy(); }
    else yilan.pop();
    ciz();
  }
  function yon(y) {
...
function retroKus(durak, index) {
  const alan = retroKabuk("🐤 Uçan Kuş", "Ekrana dokun, borulara çarpmadan geç!");
  const W = 300, H = 420;
  const cv = document.createElement("canvas");
  ...
  const G = 0.5, ZIPLA = -7.5, R = 14;
  const ARALIK = 130, GENIS = 46;
  let borular = [{ x: W, bos: 120 }];
  ...
  function adim() {
    if (bitti) return;
    vy += G; y += vy;
    borular.forEach((b) => (b.x -= 2.4));
    if (borular[borular.length - 1].x < W - 170)
      borular.push({ x: W + 20, bos: 60 + Math.random() * (H - ARALIK - 120) });
    borular = borular.filter((b) => b.x > -GENIS);
    for (const b of borular) {
      if (!b.gecti && b.x + GENIS < 60) { b.gecti = true; skor++; tonCal([700]); }
      const cx = 60;
      if (cx + R > b.x && cx - R < b.x + GENIS && (y - R < b.bos || y + R > b.bos + ARALIK)) {
        bitti = true; sesYanlis();
    ...
    borular.forEach((b) => { ctx.fillRect(b.x, 0, GENIS, b.bos); ctx.fillRect(b.x, b.bos + ARALIK, GENIS, H); });
...
function retroTugla(durak, index) {
  const alan = retroKabuk("🧱 Tuğla Kır", "Çubuğu sürükle, topu kaçırma, tüm tuğlaları kır!");
  const W = 300, H = 380;
  const cv = document.createElement("canvas");
  cv.width = W; cv.height = H; cv.className = "retro-canvas";
  alan.appendChild(cv);
  ...
  let px = W / 2 - PW / 2;
  let bx = W / 2, by = H - 40, bvx = 3, bvy = -3, skor = 0, bitti = false;
  ...
  cv.addEventListener("pointerdown", (e) => { e.preventDefault(); tasi(e.clientX); });
  cv.addEventListener("pointermove", (e) => { if (e.buttons || e.pressure) { e.preventDefault(); tasi(e.clientX); } });
  ...
  function adim() {
    ...
    for (const t of tuglalar) {
      ...
      if (bx + BR > t.x && bx - BR < t.x + TG && by + BR > t.y && by - BR < t.y + TH) {
        t.kirik = true; bvy = -bvy; skor++; tonCal([600 + t.r * 40]); break;
      }
    }
...
function retroUzay(durak, index) {
  const alan = retroKabuk("🚀 Uzay Atışı", "Gemiyi sürükle, dokununca ateş et, taşları vur!");
  const W = 300, H = 420;
  const cv = document.createElement("canvas");
  cv.width = W; cv.height = H; cv.className = "retro-canvas";
  alan.appendChild(cv);
  ...
  cv.addEventListener("pointerdown", (e) => { e.preventDefault(); tasi(e.clientX); ates(); });
  cv.addEventListener("pointermove", (e) => { if (e.buttons || e.pressure) { e.preventDefault(); tasi(e.clientX); } });

  function adim() {
    if (bitti) return;
    sayac++;
    if (sayac % 28 === 0) taslar.push({ x: 20 + Math.random() * (W - 40), y: -16, hiz: 1.4 + Math.random() * 1.6 });
...
function retroSimon(durak, index) {
  ...
  function yak(i) {
    ...
    _gec(() => p.classList.remove("aktif"), 320);
  }
  function gosterDizi() {
    oyuncuSira = false; let k = 0;
    const t = _ara(() => {
      yak(dizi[k]); k++;
      if (k >= dizi.length) { clearInterval(t); _gec(() => { oyuncuSira = true; sira = 0; }, 500); }
    }, 650);
  }
  function yeniTur() {
